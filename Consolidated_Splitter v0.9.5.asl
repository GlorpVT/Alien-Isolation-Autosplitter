...
state("AI", "Epic") // Will eventually get Microsoft Version to get all versions available
{
		...
		settings.Add("allmissions", true, "All Missions", "campaign");
			...
			settings.Add("ManualSubsplit", false, "Additional Subsplits", "amsubsplit");
				...
				settings.Add("m9Splits", false, "M9 Splits", "ManualSubsplit");
					...
					settings.Add("m9Fade", false, "M9 Load Screen", "m9Splits");
		...
		settings.Add("ilsSub", false, "IL Subsplit", "ils");
			...
			settings.Add("ILManualSubsplit", false, "Additional Subsplits", "ilsSub");
				...
				settings.Add("ILm9Splits", false, "M9 Splits", "ILManualSubsplit");
					...
					settings.Add("ILm9Fade", false, "M9 Load Screen", "ILm9Splits");
...
#region Subsplit variables - Variables are only Visible if the ["Setting"] is Checked
    if(settings["loadSubsplit"] || settings["ILloadSubsplit"]){
        ...
        vars.m9Derelict = current.MainMenu >= -111.9 && current.MainMenu <= -111.7 && current.loadingScreen < 121.3 && current.loadingScreen > 121;											                // M9 Transition to Derelict
        ...
        vars.m12ElevatorDwn = current.loadingScreen > 46.3 && current.loadingScreen < 50.1 && current.MainMenu > -30 && current.MainMenu < -25 && current.FinalLevel > 4 && current.FinalLevel < 4.2;		// Elevator From Cryo Vents
    ...
    if(settings["DEVTOOLS"] == true && current.MainMenu != 0 &&(settings["dlc"] == false || settings["campaign"] == false || settings["ils"] == false || settings["CatExt"] == false)){
        vars.loading = true;
        if(settings["PlayerPos"] == true){
            vars.PlayerX = current.MainMenu;
            vars.PlayerY = current.FinalLevel;
            vars.PlayerZ = current.loadingScreen;
			
        }
        if(settings["DPS"] == true && (settings["dlc"] == false || settings["campaign"] == false || settings["ils"] == false || settings["CatExt"] == false)){
            vars.Health = current.SaveIcon;
            vars.Damaged = 0;
            if(current.SaveIcon < old.SaveIcon){vars.Damaged = (old.SaveIcon - current.SaveIcon);}
            if(current.SaveIcon > old.SaveIcon){vars.Damaged = 0;}
        }
        if(settings["TramTimer"] == true && (settings["dlc"] == false || settings["campaign"] == false || settings["ils"] == false || settings["CatExt"] == false)){
            if(settings["M2Tram"] == true){vars.m2Tram = current.MainMenu > 23.7 && current.MainMenu < 24.1 && current.loadingScreen > 52.16 && current.loadingScreen < 52.18;}			// M2 Tram Button Press
            if(settings["M8Tram"] == true){vars.m8Tram = current.MainMenu > -7.5 && current.MainMenu < -7.4 && current.loadingScreen > 39.35 && current.loadingScreen < 39.36;}			// M8 Tram Button Press
            if(settings["M10Tram"] == true){vars.m10Tram = current.MainMenu > 39.3 && current.MainMenu < 39.4 && current.loadingScreen > -28.6 && current.loadingScreen < -28.5;}			// M10 Tram Button Press
            if(settings["M11Tram"] == true){vars.m11Tram = current.MainMenu > 50.6 && current.MainMenu < 50.7 && current.loadingScreen > -22.9 && current.loadingScreen < -22.7;}		    // M11 Tram Button Press
            if(settings["M12Tram"] == true){vars.m12Tram = current.MainMenu > -10.1 && current.MainMenu < -9.9 && current.loadingScreen > 38.7 && current.loadingScreen < 38.8;}			// M12 Tram Button Press
            if(settings["M14Tram"] == true){vars.m14Tram = current.MainMenu > -5.91 && current.MainMenu < -5.9 && current.loadingScreen > -41.73 && current.loadingScreen < -41.72;}		// M14 Tram Button Press
