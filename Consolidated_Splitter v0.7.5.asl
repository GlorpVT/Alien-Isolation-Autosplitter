state("AI", "Steam") 									// Script Written by Spazdmo (Any Issues, Please contact #Support in the Alien: Isolation Discord)
{
	...
	short Save_Icon : 0x12F0C88, 0x4C, 0x8, 0x8, 0x33C;
}
state("AI", "Epic")
{
...
#region ToolTips
	...
	settings.SetToolTip("DEVTOOLS","Good Luck Getting these to work while running");
...
#endregion
	if((old.levelManagerState == 5 && current.levelManagerState == 7 || current.gameFlowState == 6) || settings["DEVTOOLS"] == true && (settings["campaign"] == true || settings["CatExt"] == true || settings["dlc"] == true || settings["ils"] == true)){vars.loading = true;} 					// Check if on Loading Screen
