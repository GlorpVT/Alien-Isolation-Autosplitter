...
#region Subsplit variables - Variables are only Visible if the ["Setting"] is Checked
    ...
    if(settings["DEVTOOLS"] == true && current.pX != 0 &&(settings["dlc"] == false || settings["campaign"] == false || settings["ils"] == false || settings["CatExt"] == false)){
        vars.loading = true;
        ...
        if(settings["TramTimer"] == true && (settings["dlc"] == false || settings["campaign"] == false || settings["ils"] == false || settings["CatExt"] == false)){
            if(settings["M2Tram"] == true){vars.m2Tram = current.pX > 23.7 && current.pX < 24.1 && current.pZ > 52.16 && current.pZ < 52.18;}			// M2 Tram Button Press
            if(settings["M8Tram"] == true){vars.m8Tram = current.pX > -7.5 && current.pX < -7.4 && current.pZ > 39.35 && current.pZ < 39.36;}			// M8 Tram Button Press
            if(settings["M10Tram"] == true){vars.m10Tram = current.pX > 39.3 && current.pX < 39.4 && current.pZ > -28.6 && current.pZ < -28.5;}			// M10 Tram Button Press
            if(settings["M11Tram"] == true){vars.m11Tram = current.pX > 50.6 && current.pX < 50.7 && current.pZ > -22.9 && current.pZ < -22.7;}		    // M11 Tram Button Press
            if(settings["M12Tram"] == true){vars.m12Tram = current.pX > -10.1 && current.pX < -9.9 && current.pZ > 38.7 && current.pZ < 38.8;}			// M12 Tram Button Press
            if(settings["M14Tram"] == true){vars.m14Tram = current.pX > -5.91 && current.pX < -5.9 && current.pZ > -41.73 && current.pZ < -41.72;}		// M14 Tram Button Press
